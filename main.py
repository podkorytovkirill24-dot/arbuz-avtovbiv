...
def _zagruzit_blok(imya_fayla: str, obyazatelno: bool = True) -> bool:
    put = _PAPKA_FUNKCIY / imya_fayla
    print(f"Loading block: {put}")
    if not put.exists():
        prefiks = imya_fayla.split("_", 1)[0]
        kandidaty = sorted(_PAPKA_FUNKCIY.glob(f"{prefiks}_*.py"))
        if kandidaty:
            put = kandidaty[0]
            print(f"Block not found by exact name, using fallback: {put.name}")
        elif obyazatelno:
            dostupnye = ", ".join(p.name for p in sorted(_PAPKA_FUNKCIY.glob("*.py"))[:10])
            raise FileNotFoundError(
                f"Block file not found: {put}. Available examples: {dostupnye}"
            )
        else:
            print(f"Optional block missing, skip: {put}")
            return False
...
_zagruzit_blok('002_parsing_admin_aydi.py', obyazatelno=False)

if "_parse_admin_ids" not in globals():
    print("Fallback _parse_admin_ids loaded from main.py")
...
SUBMIT_RULES_TEXT = (
    "📌 Принимаем номера MAX РФ/КЗ.\n"
    "Формат: +7XXXXXXXXXX, 7XXXXXXXXXX или 8XXXXXXXXXX.\n"
    "Примеры: РФ 79991234567, КЗ 77071234567\n"
